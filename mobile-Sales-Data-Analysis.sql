...
FACT_TRANSACTIONS AS FT
ON L.IDLocation = FT.IDLocation JOIN
...
---	3. Show the number of transactions for each model per zip code per state. 
	...
	DIM_MODEL AS M ON FT.IDMODEL = M.IDMODEL JOIN 
	DIM_LOCATION AS L ON FT.IDLOCATION = L.IDLOCATION
...
---5. Find out the average price for each model in the top5 manufacturers in terms of sales quantity and order by average price. 
	...
	SELECT TOP 5 IDMANUFACTURER AS TOP_5_IDMANUFACTURER, MODEL_NAME, 
	ROUND(AVG(TOTALPRICE),2) AS AVERAGE_PRICE, COUNT(QUANTITY) AS SALES_QUANTITY
	FROM DIM_MODEL AS M JOIN FACT_TRANSACTIONS AS FT
	ON M. IDMODEL = FT. IDMODEL GROUP BY MODEL_NAME, IDMANUFACTURER
...
----6. List the names of the customers and the average amount spent in 2009, where the average is higher than 500 
	...
	FROM DIM_CUSTOMER AS C JOIN FACT_TRANSACTIONS AS FT
	ON C.IDCUSTOMER = FT.IDCUSTOMER JOIN
	DIM_DATE AS D ON FT.DATE = D.DATE
	WHERE [YEAR] = 2009 GROUP BY C.CUSTOMER_NAME, D.[YEAR]
	HAVING AVG(TOTALPRICE) > 500



--Q7--BEGIN
	-----7. Show the manufacturers that sold cellphones in 2010 but did not in 2009. 

	
	SELECT M. IDMANUFACTURER FROM DIM_MODEL AS M JOIN
	FACT_TRANSACTIONS AS FT ON M.IDMODEL = FT.IDMODEL
	WHERE DATEPART(YEAR,[DATE]) = '2010' EXCEPT
	SELECT M. IDMANUFACTURER FROM DIM_MODEL AS M JOIN
	FACT_TRANSACTIONS AS FT ON M.IDMODEL = FT.IDMODEL
...
--Q8--BEGIN
	---- 8. Find top 10 customers and their average spend, average quantity by each year. Also find the percentage of change in their spend. 


WITH TOP_10_CUSTOMER AS ( SELECT * FROM( SELECT DENSE_RANK() OVER (PARTITION BY YEAR(DATE) 
ORDER BY YEAR(DATE), IDCUSTOMER, AVG(TOTALPRICE) DESC, AVG(QUANTITY) DESC) AS RANKS,
YEAR(DATE) AS YEARS, IDCUSTOMER, AVG(TOTALPRICE) AS AVG_SPEND, AVG(QUANTITY) AS AVG_QUANTITY
FROM FACT_TRANSACTIONS GROUP BY YEAR(DATE) , IDCUSTOMER) AS X WHERE RANKS <=10 ),
YEAR_WISE_SPEND AS ( SELECT *, LAG(AVG_SPEND, 1) OVER(PARTITION BY IDCUSTOMER ORDER BY YEARS, AVG_SPEND) AS PREV_SPEND
FROM TOP_10_CUSTOMER WHERE IDCUSTOMER IN (SELECT IDCUSTOMER FROM TOP_10_CUSTOMER))
SELECT *, (AVG_SPEND - PREV_SPEND) / AVG_SPEND * 100 AS [%AGE_SPEND_CHANGE] FROM YEAR_WISE_SPEND
